...
const Container = styled.div`
    ...
    width: ${props => props.type ? '20%' : 'calc(25% - 11px)'};
    aspect-ratio: ${props => props.type ? 'none' : '1 / 1'};
    display: flex;
...
function SpecificItem({ item, message="More items to consider", type}) {
  ...
  return (
      <Container type={type}>
        ...
        <Link style={{maxWidth:"80%", maxHeight: "70%"}} className='link' to={route}><Image src={item.Data.main_image.link} alt="" /></Link>
        {!type && <Link style={{ margin: "auto 0 .5rem 0" }} className='link' to={route}><NormalLink href="#">See more</NormalLink></Link>}
          {type && <InfoContainer>
            <Link style={{textDecoration:"none", color:"black"}} to={route}>< Title > {item.Data.title}</Title></Link>
              <ReviewContainer>
